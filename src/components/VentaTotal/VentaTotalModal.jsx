...
export default function VentaTotalModal({
  open,
  onClose,
  onSuccess,
      ...
      setForm({
        ...
        PresMen:
          initialValues?.PresMen != null
            ...
            : '',
        VentMen:
          initialValues?.VentMen != null
            ? formatNumber(initialValues.VentMen)
            : '',
      });
    } else {
      const per =
  ...
  const fieldErrors = useMemo(() => {
    ...
    fe.VentMen = validateNumberField('Venta mensual', form.VentMen);
    return fe;
  }, [form]);
  ...
  const handleSave = async () => {
    ...
    const numbers = {
      ...
      VentMen: parseInputNumber(form.VentMen) ?? 0,
    };
    console.log(numbers);
    try {
      setSaving(true);
  ...
  return (
    ...
    >
      <DialogTitle>
        {isEdit ? 'Editar Venta Total' : 'Registrar Venta Total'}
      </DialogTitle>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={4}>
              <TextField
                ...
                type="month"
                fullWidth
                value={formatNumber(form.periodo)}
            ...
            <Grid item xs={12} sm={4}>
              <TextField
                label="Venta mensual"
                fullWidth
                value={formatNumber(form.VentMen)}
                onChange={onChangeNumber('VentMen')}
                onKeyDown={numericKeyFilter}
                onBlur={() => markTouched('VentMen')}
                error={showErr('VentMen')}
                helperText={helpText('VentMen', 'Ej: 17 822 897.85')}
                inputProps={{ inputMode: 'decimal' }}
              />
            </Grid>
          </Grid>
