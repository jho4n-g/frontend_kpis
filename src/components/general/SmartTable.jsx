...
export default function ReusableKpiTable({
  ...
  useEffect(() => {
    setVisibleCols((prev) => {
      const base = getDefaultVisible(); // todas visibles por defecto
      const next = { ...base, ...prev }; // respeta lo guardado pero añade nuevas
      if (columns[0]?.id) next[columns[0].id] = true; // fuerza la 1ª visible
      return next;
    });
    setPinnedLeft((prev) => prev.filter((id) => colById[id]));
    setPinnedRight((prev) => prev.filter((id) => colById[id]));
  }, [columns, colById]);
  ...
  const handleChangeMeta = () => onChangeMeta?.();
